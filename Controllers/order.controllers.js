...
const updateOrder = async (req, res) => {
  ...
  try {
    ...
    if (!order)
      ...
      if(order?.companyId != req.user.companyId.value) return createError(res, 400, "This order is not generated by your company")
...
const driverRecievedOrder = async (req, res) => {
  ...
  try {
    ...
    if (!order)
      return res.status(200).json({
        success: false,
        error: { message: `No such order found! For station or Vendor ${order}` },
      });
    if (order.driverId != driverId)
      return res.status(200).json({
        success: false,
