  ...
  cron.schedule("0 0 0 * * *", async () => {
    try {
      ...
      await Promise.all(allStations.map(async (station) => {
        ...
        const todayOrders = await Order.find({ ...query, stations: { $elemMatch: { id: station._id } } });
        let stationFuelsRecievedOrders = [];
        let stationFuelsPriceOrders = [];
        await Promise.all(todayOrders.map(async (order) => {
          await Promise.all(order.stations.map(({ id, fuel_value, fuel_recieved }) => {
            if (id == station._id) {
        ...
        await Promise.all(stationFuelsPriceOrders?.map((fuel_price) => {
        ...
        await Promise.all(todaySales?.map(({ amount }) => (saleTotalPrice += amount)));
        let saleTotalFuel = 0;
        await Promise.all(todaySales?.map(({ fuel_value }) => (saleTotalFuel += fuel_value)));
...
const stationRealTimeData = async () => {
  cron.schedule("1 * * * * *", async () => {
    try {
      ...
      await Promise.all(premiumCompanies.map(async (company) => {
        const stations = await Station.find({ companyId: company._id }).populate("fuels");
